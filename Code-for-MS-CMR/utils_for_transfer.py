...
class VAE(nn.Module):
    def __init__(self, KERNEL=3,PADDING=1):
        ...
        self.down4fc1 = nn.Sequential(Spatial_Attention(256),
                                      nn.InstanceNorm2d(256),
                                      nn.Tanh())
        self.down4fc2 = nn.Sequential(Spatial_Attention(256),
                                      nn.InstanceNorm2d(256),
                                      nn.Tanh())
        ...
        self.down2fc1 = nn.Sequential(Spatial_Attention(128),
                                      nn.InstanceNorm2d(128),
                                      nn.Tanh())
        self.down2fc2 = nn.Sequential(Spatial_Attention(128),
                                      nn.InstanceNorm2d(128),
                                      nn.Tanh())
        ...
        self.fc1 = nn.Sequential(Spatial_Attention(64),
                                 nn.InstanceNorm2d(64),
                                 nn.Tanh())
        self.fc2 = nn.Sequential(Spatial_Attention(64),
                                 nn.InstanceNorm2d(64),
                                 nn.Tanh())
...
class Decoder(nn.Module):
    def __init__(self,fb=16 ):
        ...
        self.conv_seq3 = nn.Sequential(nn.Conv2d(64,1,kernel_size=3, padding=1),
...
class Feature_Distribution_Distance_Func(torch.autograd.Function):
    ...
    def forward(ctx, feature_modala, feature_modalb, Tvalue):
        sizea = feature_modala.size(0)
        sizeb = feature_modalb.size(0)
        c = feature_modala.size(1)
        num_aa = 1 / float(sizea * sizea * c)
        num_ab = 1 / float(sizea * sizeb * c)
        num_bb = 1 / float(sizeb * sizeb * c)
        ...
        loss = num_aa*dmatrix_xx + num_bb*dmatrix_yy - 2.0 * num_ab*dmatrix_xy
...
class Feature_Distribution_Distance_func(nn.Module):
    def __init__(self, avg_kernel, avg_stride):
        ...
        self.Tvalue = 1e6  # 1000000.0 #torch.tensor(1e6).type(torch.cuda.HalfTensor)
    ...
    def forward(self, feature_modala, feature_modalb):
        ...
        feature_modala = feature_modala.permute(0,2,3,1).contiguous()
        feature_modalb = feature_modalb.permute(0,2,3,1).contiguous()
        asize = feature_modala.size()
        bsize = feature_modalb.size()
        feature_modala = feature_modala.view( -1,asize[3], 1, 1)
        feature_modalb = feature_modalb.view( -1,bsize[3], 1, 1)
