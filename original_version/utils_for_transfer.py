...
class InfoNet(nn.Module):
    def __init__(self, KERNEL=3, PADDING=1):
        ...
        self.info_seq=nn.Sequential(nn.Linear(1024*8*8,256),
                                    nn.ReLU(inplace=True),
                                    nn.Linear(256,6))
    def forward(self, z):
...
class LGE_TrainSet0(Dataset):
    def __init__(self):
        self.imgdir='/home/wfp/2019TMI/LGE_C0_T2/Original/Patch192/LGE/'
...
class C0_TrainSet0(Dataset):
    def __init__(self):
        self.imgdir = '/home/wfp/2019TMI/LGE_C0_T2/Original/Patch192/C0/'
...
def Hausdorff_compute(pred,groundtruth):
    ...
    for i in range(3):
        ...
        groundtruth_i = (groundtruth==i).astype(np.float32)
        if np.sum(pred_i)==0:
            avghausdorff_i=0.0
            hausdorff_i=0.0
            dice_i=0.0
        else:
            labelPred = sitk.GetImageFromArray(pred_i, isVector=False)
            labelTrue = sitk.GetImageFromArray(groundtruth_i, isVector=False)

            hausdorffcomputer = sitk.HausdorffDistanceImageFilter()
            hausdorffcomputer.Execute(labelTrue>0.5 , labelPred>0.5)
            avghausdorff_i =hausdorffcomputer.GetAverageHausdorffDistance()
            hausdorff_i = hausdorffcomputer.GetHausdorffDistance()

            dicecomputer = sitk.LabelOverlapMeasuresImageFilter()
            dicecomputer.Execute(labelTrue>0.5, labelPred>0.5)
            dice_i=dicecomputer.GetDiceCoefficient()

        avghausdorff = avghausdorff + [avghausdorff_i]
        hausdorff = hausdorff +[hausdorff_i]
        dice = dice+[dice_i]

    return np.array(avghausdorff,dtype=np.float32),np.array(hausdorff,dtype=np.float32),np.array(dice,dtype=np.float32)
...
class Feature_Distribution_Distance_func(nn.Module):
    def __init__(self, avg_kernel, avg_stride):
        ...
        self.Tvalue = 1e3  # 1000000.0 #torch.tensor(1e6).type(torch.cuda.HalfTensor)
